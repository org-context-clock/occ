...
;; occ-config.el holds the `occ' customization group and the runtime
;; switches of OCC (support layer): the defcustoms `occ-prefix-key',
;; `occ-unnamed', `occ-clockout-unassociable-to-unnamed',
;; `occ-completing-read-function', `occ-clock-in-ctx-auto-select-if-only'
;; and `occ-config-ineq-const-value' for the priority solver, plus
;; `occ-config-clock-in' with its enable/disable commands, `occ-confirm',
;; `occ-dev-dir', and the quiet-mode commands generated by
;; `occ-gen-binary-option-commands'.
;;
